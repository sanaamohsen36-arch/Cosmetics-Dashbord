-- Idempotent production bootstrap. Safe to run multiple times.
-- Creates every table the app queries (sales_raw_files, sales_by_salesperson,
-- sales_by_platform, ads_raw_files, meta_ads, tiktok_ads, platform_settings,
-- ocr_page_corrections, ocr_salesperson_corrections, salespeople, platforms,
-- brands, column_mappings, daily_summary, profiles, audit_log, backup_runs,
-- system_health_status, notifications), their indexes, RLS policies, and
-- seeds one Owner profile.
--
-- Root cause this fixes: the production Supabase project never had this
-- schema applied, so every REST call the app makes 404s (profiles,
-- platforms, salespeople, notifications, column_mappings, backup_runs,
-- audit_log, ocr_salesperson_corrections, ocr_page_corrections,
-- system_health_status) - login succeeds (auth.users is Supabase-managed),
-- but the app then can't read/create a profile row, so it never redirects.
...
-- Initial Owner profile. Matches by auth.users.email - edit the email below
-- if the account you log in with is different. Idempotent: safe to re-run,
-- always (re)promotes that user to 'owner' without touching anyone else.
insert into public.profiles (id, display_name, role, active)
select id, coalesce(raw_user_meta_data->>'name', email, 'Owner'), 'owner', true
from auth.users
where email = 'healthypharmaplussora@gmail.com'
